...
class SecurePRNG:
    ...
    def generate(self, n_bytes):
        ...
        while len(output) < n_bytes:
            ...
            block = hashlib.sha256(self.state + b"stream").digest()
            output += block

            # 2. Update state immediately after with a hash function (One-way progression)
            self.state = hashlib.sha256(self.state + b"update").digest()
    ...
    def intercept(self, sender, recipient, payload):
        ...
        if isinstance(payload, bytes):
            ...
            if sender == "Alice" and self.alice_prng and self.bob_prng:
                ...
                print_info("Mallory plaintext", plaintext.decode(errors="ignore"))
                ...
                return xor_crypt(modified_plaintext, self.bob_prng)
...
def main():
    ...
    message = b"Let's meet at Secret Location tomorrow!" # Put in your test message here
    ...
    if final_message == message:
        print("\n[SUCCESS] Secure transmission successful: Message integrity and confidentiality preserved.")
    else:
        print("\n[ERROR] Message mismatch detected.")
